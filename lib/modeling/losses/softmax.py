...
class CrossEntropyLoss(BaseLoss):
    ...
    def forward(self, logits, labels):
        """
            ...
            labels: [n]
        """
        logits = logits.permute(1, 0, 2).contiguous()  # [n, v, c] -> [p, n, c]
        p, _, c = logits.size()
        log_preds = F.log_softmax(logits * self.scale, dim=-1)  # [v, n, c]
        one_hot_labels = self.label2one_hot(
            labels, c).unsqueeze(0).repeat(p, 1, 1)  # [v, n, c]
        loss = self.compute_loss(log_preds, one_hot_labels)
        self.info.update({'loss': loss.detach().clone()})
        if self.log_accuracy:
            pred = logits.argmax(dim=-1)  # [v, n]
            accu = (pred == labels.unsqueeze(0)).float().mean()
            self.info.update({'accuracy': accu})
        return loss, self.info

    def compute_loss(self, predis, labels):
        softmax_loss = -(labels * predis).sum(-1)  # [v, n]
        losses = softmax_loss.mean(-1)

        if self.label_smooth:
            smooth_loss = - predis.mean(dim=-1)  # [v, n]
            smooth_loss = smooth_loss.mean()  # [v]
