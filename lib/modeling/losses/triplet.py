...
class TripletLoss(BaseLoss):
    ...
    def forward(self, embeddings, labels):
        # embeddings: [n, v, c], label: [n]
        embeddings = embeddings.permute(
            1, 0, 2).contiguous()  # [n, v, c] -> [v, n, c]
        embeddings = embeddings.float()

        ref_embed, ref_label = embeddings, labels
        dist = self.ComputeDistance(embeddings, ref_embed)  # [v, n1, n2]
    ...
    def ComputeDistance(self, x, y):
        """
            ...
            y: [v, n_y, c]
        """
        x2 = torch.sum(x ** 2, -1).unsqueeze(2)  # [v, n_x, 1]
        y2 = torch.sum(y ** 2, -1).unsqueeze(1)  # [v, 1, n_y]
        inner = x.matmul(y.transpose(-1, -2))  # [v, n_x, n_y]
        dist = x2 + y2 - 2 * inner
        dist = torch.sqrt(F.relu(dist))  # [v, n_x, n_y]
        return dist
